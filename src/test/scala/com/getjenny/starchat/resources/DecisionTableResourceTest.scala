...
class DecisionTableResourceTest extends WordSpec with Matchers with ScalatestRouteTest with JsonSupport {
  ...
  //search
  it should {
    "return an HTTP code 200 when search successfully executed" in {
      val decisiontablesearch: DTDocumentSearch = DTDocumentSearch(
        from = Some(0),
        size = Some(10),
        executionOrder = Some(1),
        minScore = Some(1),
        boostExactMatchFactor = Some(1000),
        state = Some("renew_insurance"),
        evaluationClass = Some("default"),
        queries = Some("hello world"),
        searchAlgorithm = Some(SearchAlgorithm.NGRAM3)
      )
      Post(s"/index_getjenny_english_0/decisiontable/search", decisiontablesearch) ~> addCredentials(testUserCredentials) ~> routes ~> check {
        status shouldEqual StatusCodes.OK
      }
    }
  }
  ...
  //get next response
  it should {
    "return HTTP code 200 when getting next response" in {
      val getnextresponse: ResponseRequestIn = ResponseRequestIn(
        conversationId = "conv_12131",
        traversedStates = Some(Vector[String]("state_0", "state_1", "state_3")),
        userInput = Some(ResponseRequestInUserInput(text = Some("Hello, I would like to reset my password, is it possible?"), img = Some("aGVsbG8K"))),
        state = Some("welcome_state"),
      ...
      Post(s"/index_getjenny_english_0/get_next_response", getnextresponse) ~> addCredentials(testUserCredentials) ~> routes ~> check {
        status shouldEqual StatusCodes.OK
      }
    }
  }
