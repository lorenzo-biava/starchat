...
class TokenizerResourseTest extends WordSpec with Matchers with ScalatestRouteTest with JsonSupport {
  ...
  it should {
    "return an HTTP code 200 when tokenizer request successfully executed" in {
      val tokenizers: TokenizerQueryRequest = TokenizerQueryRequest (
        tokenizer = "base",
        text = "good morning, please tokenize this text"
      )
      Post(s"/index_getjenny_english_0/tokenizers", tokenizers) ~> addCredentials(testUserCredentials) ~> routes ~> check {
