...
class GteOperator(child: List[Expression]) extends AbstractOperator(child: List[Expression]) {
  ...
  def add(e: Expression, level: Int = 0): AbstractOperator = {
    if (level === 0) new GteOperator(e :: child)
    else if (level === 1) {
      child.headOption match {
        case Some(t) =>
          t match {
            case c: AbstractOperator => new GteOperator(c.add(e, level - 1) :: child.tail)
            case _ => throw OperatorException("GteEqualOperator: trying to add to smt else than an operator")
          }
        case _ =>
          throw OperatorException("GteEqualOperator: trying to add None instead of an operator")
      }
    } else {
      throw OperatorException("GteEqualOperator: trying to add more than two expression")
    }
  }
