...
/** Basically a word separated from the others. E.g.:
  ...
  */
class EuclideanDistanceAnalyzer(val arguments: List[String]) extends AbstractAtomic {
  override def toString: String = "eucDistanceKeywords(\"" + arguments + "\")"
  val isEvaluateNormalized: Boolean = true
  private val rx = {"""\b""" + arguments + """\b"""}.r
  def evaluate(query: String, data: Data = Data()): Result = {
    ...
    if (freq > 0) println("DEBUG: KeywordAtomic: '" + arguments + "' found " + freq +
